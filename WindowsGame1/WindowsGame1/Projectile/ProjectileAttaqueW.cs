...
namespace AtelierXNA
{
    ...
    public class ProjectileAttaqueW : Projectile, IDestructible
    {
        ...
        public ProjectileAttaqueW(Game game, string nomModËle, float ÈchelleInitiale, Vector3 rotationInitiale, Vector3 positionInitiale,
                                       Vector3 direction,Vector3 directionDÈplacement, int force, int prÈcision,float intervalleMAJ)
        ...
        void GestionDÈplacement()
        {
            ...
            if (!(¿DÈtruire) && distanceEntre2Positions <= PORTEE_MAX)
            {
                if (DirectionDÈplacement.X >= 0 || DirectionDÈplacement.X <= 0)
                {
                    ...
                    foreach(EntitÈ entitÈ in Game.Components.Where(x => x is EntitÈ) )
                    {
                        if(BoiteDeCollision.Intersects(entitÈ.BoiteDeCollision)&& !entitÈ.EstAlliÈe)
                        {
                            Cible = entitÈ;
                            ¿DÈtruire = true;
                        }
